...
def load_dks_forward_premium() -> pd.DataFrame:
    ...
    out = out.dropna(subset=["date", "dks_forward_premium_3m_pct_ann"])
    out = out.rename(columns={"currency": "ccy"})
    out["date"] = out["date"].dt.to_period("M").dt.to_timestamp("M")
    return out[["date", "ccy", "dks_forward_premium_3m_pct_ann"]].drop_duplicates(["date", "ccy"])
...
def plot_comparison(df: pd.DataFrame) -> None:
    ...
    monthly = df.groupby("date")[["futures_premium_fixed_3m_pct_ann", "dks_forward_premium_3m_pct_ann"]].median()
    ax.plot(monthly.index, monthly["dks_forward_premium_3m_pct_ann"], color=PALETTE[0], label="DKS 3m forward premium")
    ax.plot(monthly.index, monthly["futures_premium_fixed_3m_pct_ann"], color=PALETTE[1], label="Yahoo futures proxy, fixed 3m")
    ax.axhline(0, color=PALETTE[8], lw=0.8, alpha=0.7)
    ax.set_title("Forward Premium Comparison")
    ...
    note = (
        ...
        "The futures proxy is annualized from log(F/S); DKS rho is the benchmark 3-month forward premium."
    )
...
def plot_gap(df: pd.DataFrame) -> None:
    set_custom_style()
    summary = df.groupby("date")["premium_gap_fixed_3m_pct_ann"].agg(
        median="median",
    ...
    ax.axhline(0, color=PALETTE[8], lw=0.8, alpha=0.7)
    ax.set_title("Futures Proxy minus DKS Forward Premium")
    ax.set_ylabel("Annualized percentage-point gap")
    ax.xaxis.set_major_locator(mdates.YearLocator(3))
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    ax.legend(loc="upper left")
    note = "Source: Author's calculations. Positive values mean the Yahoo futures proxy implies a higher annualized premium than DKS rho."
    fig.text(0.10, 0.025, note, ha="left", va="bottom", fontsize=6.5, color=PALETTE[8], wrap=True)
    plt.subplots_adjust(left=0.13, right=0.97, top=0.86, bottom=0.22)
...
def main() -> None:
    ...
    print(comparison[['futures_premium_fixed_3m_pct_ann','dks_forward_premium_3m_pct_ann','premium_gap_fixed_3m_pct_ann']].describe().round(4).to_string())
